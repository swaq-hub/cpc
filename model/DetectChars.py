...
def loadKNNDataAndTrainKNN():
    ...
    try:
        npaClassifications = np.loadtxt("./model/training_dataset/classifications.txt", np.float32)                  # read in training classifications
    ...
    try:
        npaFlattenedImages = np.loadtxt("./model/training_dataset/flattened_images.txt", np.float32)                 # read in training images
...
def findPossibleCharsInPlate(imgGrayscale, imgThresh):
    ...
    imgContours, contours = cv2.findContours(imgThreshCopy, cv2.RETR_LIST, cv2.CHAIN_APPROX_SIMPLE)

    for contour in imgContours:                        # for each contour
